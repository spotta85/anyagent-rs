...
├── error.rs         AgentError
├── catalog.rs       per-agent facts as data: ids, launch flags, login markers
├── discovery.rs     finds executables and login markers; never launches
...
| Step | Where | What |
|---|---|---|
| 1 | `src/catalog.rs` | A profile: id, executable names, args for protocol mode, login markers, install hint, config-home env var. Only flags verified against a real install |
...
catalog entry is only needed for verified quirks and login markers.
