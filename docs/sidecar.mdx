...
- A session error is terminal: `closed` follows it, after the engine's last bookkeeping events.
- `closed` is written exactly once per opened session.
- EOF on stdin closes every open session, writes their `closed`, and exits 0. Closing is "close stdin, wait".
...
| `generate` | agent, dir, prompt | string |
...
`tests/schema.rs`. The command type is `Frame`; events are `Event` and
`EventKind`; errors are `ErrorBody`. Feed it to a generator:
