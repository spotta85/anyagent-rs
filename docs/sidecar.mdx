...
for await (const ev of session.events()) {
  if (kindOf(ev) === "TextDelta") process.stdout.write(ev.kind.TextDelta.text);
  if (kindOf(ev) === "RequestOpened") await session.answer(ev.kind.RequestOpened.Permission.id, { Permission: "AllowOnce" });
...
Every package passes the same twenty tests: ten over the mock scripts on
macOS, Linux, and Windows in CI, ten live on claude and codex.
...
honest, and the same ten subprocess tests and ten live tests run in each.
