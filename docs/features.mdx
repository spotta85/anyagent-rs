...
login markers. Agents that are supported but missing come back with an
install hint.
...
**Confirms:** the report itself. `agent.auth` is a best-effort offline guess;
use probe for the truth.
