...
fn wrapper(name: &str, flags: &str) -> PathBuf {
    ...
    let fixture =
        Path::new("/tmp/antigravity-review-fixture.mjs").to_path_buf();
...
/// Killed tools must settle instead of appearing to keep running.
#[tokio::test]
async fn review_cancel_settles_killed_tools() {
    let (session, mut events) = open("review-cancel").await;
    session.prompt("sleep").await.unwrap();
    while !matches!(next(&mut events).await.kind, EventKind::ToolUpdated(_)) {}
    session.cancel(true).await.unwrap();
    let kinds = drain_turn(&mut events).await;
    session.close().await.unwrap();
    assert!(matches!(kinds.last(), Some(EventKind::TurnEnded { background, .. }) if background.is_empty()), "killed tools still running: {kinds:?}");
}

/// A cancelled turn's usage cannot be attributed to the next failed turn.
#[tokio::test]
async fn review_cancel_clears_turn_usage() {
    let (session, mut events) = open("review-usage").await;
    session.prompt("sleep").await.unwrap();
    while !matches!(next(&mut events).await.kind, EventKind::ToolUpdated(_)) {}
    session.cancel(true).await.unwrap();
    ...
    let kinds = drain_turn(&mut events).await;
    session.close().await.unwrap();
    assert!(!kinds.iter().any(|k| matches!(k, EventKind::ContextUsage { .. })), "stale usage on next turn: {kinds:?}");
}

/// The recorded capitalized ACP auth refusal remains a typed login error.
#[tokio::test]
async fn review_upgrade_auth_failure_is_typed() {
    let fixture = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/acp/fixture.mjs");
    let mut agent = AgentInstallation::acp("antigravity", "node", vec![fixture.display().to_string(), "--auth-required".into(), "--capitalized-auth".into(), "--no-auth-methods".into()]);
    agent.source = anyagent::InstallationSource::KnownLocation;
    let result = Runtime::new().open(&agent, SessionOptions::in_dir(std::env::temp_dir())).await;
    let err = result.err().expect("must refuse");
    assert!(matches!(err, AgentError::AuthRequired { .. }), "auth refusal became: {err:?}");
}
